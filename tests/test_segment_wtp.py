...
def test_choice_target_code_runs():
    ...
    df_coded = pcc.encode(
        df,
        reference_levels={"price": 10, "os": "android", "camera": "standard"},
    )
    result = pcc.fit(
        df_coded,
        choice="choice",
...
def test_choice_2level_dummy_matches_numeric():
    df = _simulate_cbc({6: 0.0, 10: -1.2}, {"A": 0.0, "B": 0.5}, n_sets=40_000, seed=3)
    # (A) ダミー価格
...
def test_choice_3level_segment_slopes():
    ...
    df = _simulate_cbc(
        {6: 0.0, 8: -0.3, 10: -1.6}, {"A": 0.0, "B": 0.6}, n_sets=60_000, seed=5
    )
...
def test_choice_price_segment_filter():
    df = _simulate_cbc(
        {6: 0.0, 8: -0.3, 10: -1.6}, {"A": 0.0, "B": 0.6}, n_sets=20_000, seed=6
    )
    dc = pcc.encode(df, reference_levels={"price": 6, "brand": "A"})
    result = pcc.fit(
        dc,
...
def test_rating_2level_matches_legacy_formula():
    ...
    assert np.isclose(
        w.loc["os_0", "限界支払意思額"], w_lin.loc["os_0", "限界支払意思額"]
    )
...
def test_price_col_none_message_choice():
    df = _simulate_cbc({6: 0.0, 10: -1.0}, {"A": 0.0, "B": 0.4}, n_sets=2000, seed=2)
    dc = pcc.encode(df, reference_levels={"price": 10, "brand": "A"})
    result = pcc.fit(
        dc,
        choice="choice",
...
def test_constructive_matching_choice():
    ...
    df = _simulate_cbc({6: 0.0, 10: -1.0}, {"A": 0.0, "B": 0.4}, n_sets=8000, seed=8)
    # price_range（別属性）を追加：価格が安いと "low" 帯
    df["price_range"] = np.where(df["price"] <= 6, "low", "high")
    dc = pcc.encode(
        df, reference_levels={"price": 10, "brand": "A", "price_range": "high"}
    )
    result = pcc.fit(
        dc,
        choice="choice",
...
def test_price_segment_rejected_for_single_segment_choice():
    ...
    df = _simulate_cbc({6: 0.0, 10: -1.0}, {"A": 0.0, "B": 0.4}, n_sets=2000, seed=13)
...
def test_encode_keeps_numeric_price_rating():
    df = pd.DataFrame(
        {"rating": [5, 3, 6, 4], "price": [6, 10, 6, 10], "os": ["a", "b", "b", "a"]}
    )
