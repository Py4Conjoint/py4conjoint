...
### Changed
- rating の `suggest_n_profiles()`：**要約を印字する**ようにした（choice の `suggest_n_respondents()` と揃えた振る舞い。従来この関数は何も印字しなかった）。印字するのは (1) 推奨値の根拠となる3基準（統計的最低限 p / Orme (2010) の経験則 = 符号化列数 × 2 / 観測数条件）、(2) 推奨された n_profiles で**水準バランスと D 最適性が両立するか**の2点。(2) は `design_profiles(auto_balance=True)` を実際に実行して判定し、「両立する（auto_balance は不要）」「両立しない（`auto_balance=True` でバランスは取れるが det(X'X) が 75% に下がる、など実際の比を示す）」「判定せず」を出し分ける。判定するのは候補の選び方の総数が 100,000 以下のときだけで、超える場合は「判定せず」として `design_profiles(auto_balance=True)` を実行すれば分かる旨を案内する（この上限は `design_profiles` 自身が総当たりに入る上限 1,000,000 とは**別の、より低い値**である。後者は利用者が明示的に最適化を依頼した関数の上限で数秒を許容できるが、`suggest_n_profiles` は報告する関数で、しかも待たされる理由が利用者に見えないため）。**返り値の DataFrame（列・行・値・`attrs`）は変更していない。**
- rating の `check_design()`：バランス・相関の警告に添える**案内文が状況によって変わる**ようにした。「プロファイルの組み合わせを見直してください」という助言は、その偏り・相関がそのプロファイル数では構造的に避けられない場合、実行できない指示になるため。同じ属性・水準で作れる n プロファイルの設計をすべて調べ、警告の出ない設計が存在すれば `design_profiles(..., auto_balance=True)` で解消できる旨を（実際にその関数が返す設計を同じ基準で評価したうえで）添え、存在しなければ「この n では回避できません」と明示して n_profiles を変えることを案内する。相関は「そのペアだけ避けられるか」ではなく「**相関の指摘が1つも出ない設計が存在するか**」で判定する（特定のペアの |r| を下げても別のペアに移るだけ、という状況が実際にあるため）。「相関の指摘が出ない設計は存在するが、それは水準バランスを満たさない（両立する設計が存在しない）」場合はその旨を案内する。**重大度（[大] / [中]）の判定は変更していない**（重大度は統計的な影響の大きさを表すべきで、直しやすさではないため）。判定の基準（CV・|r| の閾値、同一属性内のペアの除外）は `analysis.py` の定数とヘルパーを唯一の定義として参照している。候補の選び方の総数が 100,000 を超える場合と、水準が1つしかない属性がある場合は判定せず、従来どおりの文言のままとする（この上限は `design_profiles(auto_balance=True)` が総当たりに入る上限 1,000,000 とは別で、報告する関数が即座に返るようにより低くしてある）。
- rating の `Diagnostic` に **`hint` フィールド**（既定 `""`）を追加し、上記の案内文を `recommendation` に連結せずここに入れるようにした。`warnings()` が返す DataFrame は `recommendation` をそのままセルに入れるため、複数行の案内を連結すると改行と字下げが `\n        ` というエスケープ文字として表示されて読めなくなる。`warnings()` は **hint を独立した列**（1行に連結した形）で返し、`summary()` などのテキスト表示では従来どおり `recommendation` の下に字下げして続ける。`recommendation` の文言は変更していない。既定値があるので既存の `Diagnostic` の構築は影響を受けない（案内を付けない警告では空文字列）。
- rating の `suggest_n_profiles()`：印字に (3) **水準バランスと属性間相関の指摘がどちらも出ない n はどれか**を加えた。従来の「n_profiles を変えると両立する場合があります」は方向を示さず、増やす側にしか読めないが、実際には答えが推奨値より**小さい側**にあることがある（2水準3属性なら、指摘が出ないのは推奨の 6 ではなく n = 4 と n = 8）。判定の仕組みはすでにあるので、p から `min(N, max_burden)` までの n を実際に走査し、`design_profiles(..., auto_balance=True)` で作った設計を `check_design` と同じ基準で評価して列挙する。走査するのは C(N, n) が 100,000 以下の n だけで、超える n は調べずにその旨を印字する（1つも調べられない場合はこの行を印字せず、従来の文言のまま）。C(N, n) は n = N/2 まで増えてから減るので、上限を超えた時点で打ち切り、**すべての n について C を計算しない**（N が大きいと C の計算自体が重い）。列挙した n に**別カテゴリの警告が残る**場合（`few_profiles`）は、それが残ることを明示する（「この n にすれば警告が消えます」とだけ書くと、消えない警告が出たときに読んだ人が混乱するため）。この行を印字するときは、方向を示さない従来の一文は出さない。判定しない場合に印字する候補の組み合わせ数は、桁が大きいときは指数表記にする（8属性×4水準の構成では 96 桁になり、カンマ区切りだと 127 文字で1行を埋めてしまうため。教材規模の 10 桁程度まではカンマ区切りのまま）。**返り値の DataFrame は変更していない。**
- rating の `check_design()`：「相関の指摘が出ない設計は存在するが、それは水準バランスを満たさない」場合の案内文で、**主語がいまの設計と取り違えられない**ようにした。従来は「その設計は水準バランスを満たしません」と書いていたが、バランス表に ◎ が並んだ4行下にこの文が出るため、いまの設計の話に読めて自己矛盾に見える（教材データで確認）。「相関の指摘が出ない組み合わせ**も**存在しますが、**それらはいずれも**水準バランスを満たしません」と書き、続けて**いまの設計がどちら側なのか**（バランスのほうを満たして相関が残っているのか、どちらの指摘も出ているのか）を示す。あわせて、相関を避けられない場合に添えていた「このペアだけを 0 にすることはできますが、別のペアに移るだけです」を、**評価対象のペアが2組以上あるときだけ**「このペアの相関を下げられたとしても、指摘は別のペアに移ります」と書くようにした（属性が2つで各2水準ならペアは1組しかなく、移る先が存在しない。またこの書き方なら、ペア単位の判定をしていなくても「相関の指摘が1つも出ない設計が存在しない」ことからの含意として言える）。
- rating の `check_design()`：プロファイル数が少ないときの警告を、**n = p（飽和設計）と n = p + 1 で書き分ける**ようにした。発火条件（n < p + 2）は従来どおりだが、n = p はちょうど最小限であり、n = p + 1 とは意味が違う。n = p では「パラメータ数と同じで、ちょうど最小限です（飽和設計）」と述べ、飽和設計はプロファイルごとの平均評点をそのまま再現するため**回答者を何人増やしても交互作用やモデルの当てはまりの悪さを検出する自由度は生まれない**ことを `hint` に添える（教材の2水準3属性・4プロファイルは必ずここに当たる）。n = p + 1 の文言は従来のまま。重大度（[中]）と `category`（`few_profiles`）は変更していない。
- choice の `suggest_n_respondents()`：印字される要約行に、**最大水準数 c を決めている属性名**を添えるようにした（`最大水準数 c = 3（属性 'price'）, 設問数 t = 6, 代替案数 a = 3`）。学生が読むのは要約行であり、どの属性が必要回答者数を決めているかが分かると、設計自体を見直す判断（価格を3水準から2水準に減らせば56人で済む、など）につながるため。同じ水準数の属性が複数ある場合は、1つだけ名指しすると「その属性を減らせば人数が減る」と誤解される（他が同じ水準数のままなら c は変わらない）ので、名前を並べたうえで補足行を添える（4つ以上は3つまで並べ「ほか N 件」とする）。`c = 2` のときは水準数の下限（2未満は入力チェックで弾かれるため、必ず全属性が2水準）なので、`（全属性が2水準）` と示し、水準数では c を下げられないこと、設問数 `t` や代替案数 `a` を増やせば目安人数は下がるが**回答者1人あたりの負担が増える**ことを併記する。**返り値の DataFrame（インデックス・列・値・`attrs`）は変更していない。**

### Added
- rating の `design_profiles()` に **`auto_balance` 引数**（キーワード専用、既定 `False`）を追加。`True` にすると「すべての属性で水準の出現回数の最大と最小の差が 1 以下」という制約のもとで det(X'X) を最大化する。`suggest_n_profiles()`（推定の安定性）が推奨した n_profiles で `design_profiles()`（D 最適性）が設計を作ると、`check_design()`（水準バランス・直交性）が [大] のバランス警告を出すことがあり、その逃げ道になる。名前は choice の `design_choice_sets()` と揃えたが、**実装方式は異なる**：choice は候補を複数生成して最もバランスの良いものを選ぶのに対し、rating は均等制約下で D 最適化する。線形回帰の情報行列 X'X は係数に依存しないため厳密な最適化ができるという数学的な違いによる。候補の選び方の総数 C(N, n_profiles) が 1,000,000 以下なら**総当たりで厳密解**、超える場合は制約付き交換アルゴリズム（**発見的探索**）に切り替える。どちらだったかを含む来歴は `df.attrs["auto_balance"]`（`method` / `balanced` / `det_xpx` / `det_xpx_unconstrained` / `det_ratio`）で確認でき、`det_ratio` は「バランスを取ったことで失う精度の割合」を表す（例：2×2×2 を6プロファイルなら 0.75）。バランスを満たす設計が見つからない場合は例外にせず、警告のうえ制約なしの最良解を返す（このとき `det_ratio` は比を定義できないので `None`。`1.0` にすると「精度の損失なし」と読めてしまい、制約を満たせなかったことが伝わらないため）。**保証するのは水準バランスだけで、属性間の直交性は約束しない**（バランスを満たしていても `check_design()` が相関を指摘することはある）。既定の `False` では従来の挙動を完全に維持する。
